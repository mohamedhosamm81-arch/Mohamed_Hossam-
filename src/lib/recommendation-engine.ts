...
export class RecommendationEngine {
  ...
  private static getRecommendationReason(userProfile: Profile, candidateProfile: Profile): string {
    ...
    if (teachingMatch.length > 0 && learningMatch.length > 0) {
      return `Perfect match: Teaches ${teachingMatch[0]}, wants to learn ${learningMatch[0]}`;
    } else if (teachingMatch.length > 0) {
      return `Teaches ${teachingMatch[0]} - what you want to learn`;
    } else if (learningMatch.length > 0) {
      return `Wants to learn ${learningMatch[0]} - what you teach`;
    } else if (candidateProfile.rating >= 4.8) {
      return `Highly rated expert (${candidateProfile.rating}/5)`;
    } else {
      return `Great profile with ${candidateProfile.exchange_count} successful exchanges`;
    }
  }
