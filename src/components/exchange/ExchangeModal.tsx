...
export default function ExchangeModal({ expert, onClose }: Props) {
  ...
  const handleSubmit = async () => {
    if (!mySkill.trim() || !theirSkill.trim() || !user) return;
    setLoading(true);
    setError('');
    try {
      // Insert into exchanges table
      const { data: exchangeData, error: exchangeErr } = await supabase.from('exchanges').insert({
        sender_id: user.id,
      ...
      if (expert.user_id && exchangeData) {
        await supabase.from('notifications').insert({
          ...
          message: `New exchange proposal: ${mySkill.trim()} ↔ ${theirSkill.trim()}`,
