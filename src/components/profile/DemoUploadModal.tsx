...
export default function DemoUploadModal({ onClose, onUploaded }: Props) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to upload demo');
    } finally {
      setUploading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-6 space-y-5">
          ...
          {/* Allowed Users (only if custom visibility) */}
          {visibility === 'custom' && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-3">
              <label className="block text-sm font-medium text-amber-800">
                Allowed Usernames
              </label>
              <p className="text-xs text-amber-600">Only these users will be able to view your demo.</p>
